#!/usr/bin/env bash
# Boot-and-answer smoke test for llm-bridge-codex.
#
...
#   -discover walks Codex's rollout tree at $HOME/.codex/sessions/**/*.jsonl,
#   cold-imports each one into state.db, and emits one msg.StoredSession per
#   session. It both reads the filesystem and WRITES SQLite, so the HOME
#   sandbox below is load-bearing: run unsandboxed it writes the live state.db.
#
...
# reason: several harnesses exec their upstream CLI (codex), and a smoke
...
TMP_DIR="$(mktemp -d -t codex-e2e.XXXXXX)"
BIN_DIR="$TMP_DIR/bin"
...
step "seed a Codex rollout and prove -discover cold-imports it"
# Codex names rollouts rollout-<ts>-<uuid>.jsonl and recovers the id from the
# file body, falling back to the trailing 36 chars of the filename. Seeding one
# and reading it back exercises the walk, the SQLite write and the parse — an
# empty array alone would also be returned by a discover that had quietly died.
SEED_UUID="11111111-2222-3333-4444-555555555555"
mkdir -p "$SANDBOX_HOME/.codex/sessions/2026/01/01"
cat >"$SANDBOX_HOME/.codex/sessions/2026/01/01/rollout-2026-01-01T00-00-00-$SEED_UUID.jsonl" <<EOF
{"timestamp":"2026-01-01T00:00:00Z","type":"message","payload":{"role":"user","content":[{"type":"input_text","text":"e2e smoke seeded prompt"}]}}
EOF
